...
**Stage:** walking, with evidence. The map is issue #1 in this repo. Slice 1 (#2) drives a goal: #6 the manifest, `observe()` and the page hash, #7 the step loop, #9 the recording, #8 `jev.run` and `jev.status` behind `agent-browser plugin run`, checked against agent-browser 0.38.1. Slice 2 (#3) judges: #11 the policy engine and #14 the bug-hunt policy are on main. Slice 3 (#4) walks: #12 built the frontier walk, the fixture values and the finding dedupe, on open PR #26; #13 replays every new finding on a `<session>-repro` session for a recording and a shot per action, and `findings.json` now holds `{ findings, summary }`, on `feat/13-repro-evidence` stacked on that PR. #15 (README and SKILL.md) is the last ticket of the slice. The replay drives agent-browser 0.38.1 for real on a local fixture page; every Jev reply in the tests is still hand-written, and no manual walk with a real key has run.
