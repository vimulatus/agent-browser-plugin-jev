...
| A code over six boxes | `run "enter the code 123456 and verify" --url .../otp-no-advance.html` | No. The six fills and the Verify click are covered by replayed answers only (#100) |
| Resume after a code step | `run "sign in to Acme" --url .../otp-single.html`, then `resume <session> --value 482913` | No. The run blocks as `otp` and `resume` goes on in replayed tests only (#89) |
