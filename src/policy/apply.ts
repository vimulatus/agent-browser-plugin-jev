...
import { pageFacts, type Facts, type Gathered } from "./facts.js";
...
export function applyPolicy(policy: Policy, observation: Observation, gathered: Gathered = {}): Finding[] {
  ...
  for (const inference of gathered.inferences ?? []) {
    if (inference.over !== "finding") scopes[inference.over][inference.index][inference.question] = inference.answer;
  }
